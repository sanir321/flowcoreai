...
export default function InboxPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    const fetchedSessions = (sessionData as unknown as Session[]) || []
    setSessions(fetchedSessions)

    // AUTO-TAB SWITCH
    const hasTodo = fetchedSessions.some(s => s.status === 'escalated')
    const hasActive = fetchedSessions.some(s => s.status === 'active')
    if (!hasTodo && hasActive && activeTab === 'todo' && fetchedSessions.length === 0) {
      setActiveTab('handling')
    }

    const { data: wsData } = await supabase.from("workspaces").select("welcome_template").eq("id", workspaceId).single()
    if (wsData) setWelcomeTemplate(wsData.welcome_template || "")

    const { data: contactData } = await supabase.from("contacts").select("*").eq("workspace_id", workspaceId).limit(50)
    setContacts((contactData as unknown as Contact[]) || [])
  }, [workspaceId, activeTab, supabase])
  ...
  }, [workspaceId, fetchData, supabase])

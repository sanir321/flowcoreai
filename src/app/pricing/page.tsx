    ...
    <div className="min-h-screen" style={{ background: "#050505", ...sf }}>
      <header className="h-14 flex items-center justify-between px-6 lg:px-12 fixed top-0 left-0 right-0 z-[100]" style={{ background: "rgba(5,5,5,0.85)", backdropFilter: "blur(14px)", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
        ...
        <nav className="hidden md:flex items-center gap-1">
          {["Industries", "Customers", "Pricing", "Resources"].map((item, i) => (
            <Link
              key={item}
              href={item === "Pricing" ? "/pricing" : "/"}
              className="px-4 py-2 text-sm font-normal transition-all duration-300 rounded-lg"
              style={{ color: item === "Pricing" ? "#fff" : "#a3a3a3", background: item === "Pricing" ? "rgba(198,95,57,0.1)" : "transparent" }}
              onMouseEnter={(e) => { e.currentTarget.style.color = "#fff"; if (item !== "Pricing") e.currentTarget.style.background = "rgba(198,95,57,0.08)" }}
              onMouseLeave={(e) => { e.currentTarget.style.color = item === "Pricing" ? "#fff" : "#a3a3a3"; if (item !== "Pricing") e.currentTarget.style.background = "transparent" }}
            >
              {item}
            </Link>
          ))}
      ...
      <main className="relative overflow-hidden">
        ...
        <section className="relative z-10 flex flex-col items-center pt-36 pb-24 px-6" style={{ background: "linear-gradient(180deg, #0a0605 0%, #050505 50%)" }}>
          <motion.div
            initial={{ opacity: 0, y: 40 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 1.2, ease: [0.16, 1, 0.3, 1] as const }}
            className="max-w-[700px] mx-auto text-center space-y-10"
          >
        ...
        <section className="relative z-10 flex flex-col items-center px-6 pb-32">
          <motion.div
            ...
            className="p-8 rounded-3xl w-full max-w-md space-y-6"
            style={{ background: "rgba(10,10,10,0.6)", backdropFilter: "blur(20px)", border: "1px solid rgba(255,255,255,0.06)" }}
          >
